...
## ⚠️ 중요: 첫 실행 전 반드시 읽어주세요!

**회원가입 시 "서버 연결에 실패했습니다" 에러가 발생하는 경우:**
- 백엔드 dependencies가 설치되지 않았을 가능성이 높습니다
- **반드시** 아래 2번 단계의 `npm install`을 먼저 실행해야 합니다
- bcryptjs 등 필수 패키지가 설치되어야 회원가입이 작동합니다
- 자세한 문제 해결: [SETUP_TROUBLESHOOTING.md](./SETUP_TROUBLESHOOTING.md) 참조
...
# 2. ⚡ 의존성 설치 (필수! - 이 단계를 건너뛰면 회원가입 실패)
npm install
